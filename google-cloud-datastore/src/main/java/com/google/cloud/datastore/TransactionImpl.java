...
final class TransactionImpl extends BaseDatastoreBatchWriter implements Transaction {
  ...
  public Iterator<Entity> get(Key... keys) {
    ...
    Optional<ReadOptions> readOptions =
        this.readOptionProtoPreparer.prepare(ImmutableList.of(setTransactionId(transactionId)));
  ...
  public AggregationResults runAggregation(AggregationQuery query) {
    return datastore.runAggregation(query, setTransactionId(transactionId));
  }
